module PsipredHelper

  require "pp"
  #Takes a PSIPRED horiz file output and parses the prediction line for the residue assignments
  def parsePsipred(predictionText)
    hAnnotations = Hash.new{|h,k| h[k]=Hash.new(&h.default_proc) }

    datatext = predictionText.data()    #Ok handle the first line, and split each prediction chunk in to sections
    datatext = datatext.split("\n")[2..-1].join("\n")
    pp datatext
    #gsub(/#\sPSIPRED\sFORMAT\s\(PSIPRED\sV3\.0\)\n\n/, "")
    #datatext = datatext.gsub(/#\sPSIPRED\sHFORMAT\s\(PSIPRE\sV3\.3\)\n\n/, "")
    # PSIPRED HFORMAT (PSIPRED V3.3)
    
    alignment_segments = datatext.split(/\n{3}/)
  ...
  def parseMemsatSVM(predictionText,length)
    hAnnotations = Hash.new{|h,k| h[k]=Hash.new(&h.default_proc) }
  ...
  def parseDisopred(predictionText)
    hAnnotations = Hash.new{|h,k| h[k]=Hash.new(&h.default_proc) }
  ...
  def parseDompsi(predictionText, length)
    hAnnotations = Hash.new{|h,k| h[k]=Hash.new(&h.default_proc) }
    positions = []
  ...
  def parseDomssea(predictionText, length)
    hAnnotations = Hash.new{|h,k| h[k]=Hash.new(&h.default_proc) }
    boundarySet = []
